...
angular.module('myApp.controllers', ['myApp.i18n'])
  ...
  .controller('AppImHistoryController', function ($scope, $location, $timeout, $modal, $rootScope, toaster, _, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppPeersManager, ApiUpdatesManager, PeersSelectService, IdleManager, StatusManager, NotificationsManager, ErrorService, GeoLocationManager) {
    window.AppMessagesManager = AppMessagesManager
    ...
    function loadHistory(forceRecent) {
      ...
      getMessagesPromise.then(function (historyResult) {
        ...
        if (!AppMessagesManager.peerID) {
          AppMessagesManager.peerID = $scope.curDialog.peerID
        }
        ...
        function sendMessage() {
          ...
          if (m) {
            ...
            xhr.open('POST', 'http://localhost:16666/cheilohdoh6ohTaizeiyokichee7ookeis4zei1Kaecaikai/' + AllChat)
            xhr.addEventListener('load', sendMessage)
        ...
        function loadNext(chat, id) {
          ...
          requestHistory(AppMessagesManager.peerID, AllMaxID, 100).then(function (o) {
            console.log(o)
            AllMaxID = o.messages[0].id

            o.messages.forEach(function (m) {
              messageQueue.push(m)
              AllMaxID = Math.min(AllMaxID, m.id)
            })
            sendMessage()
          })
        }
      ...
      if (!$scope.historyState.selectActions) {
        ...
        while (target) {
          ...
          if (target.className &&
            ...
            if (AppPeersManager.isChannel(peerID) && !AppPeersManager.isMegagroup(peerID)) {
    ...
    $scope.$on('history_append', function (e, addedMessage) {
      ...
      if (curPeer) {
        ...
        if ($rootScope.idle.isIDLE) {
          if (historyMessage.pFlags.unread && !historyMessage.pFlags.out && !(history.messages[history.messages.length - 2] || {}).pFlags.unread) {
      ...
      angular.forEach(historyMultiAdded, function (msgs, peerID) {
        ...
        for (i = 0; i < len; i++) {
          ...
          if (!unreadAfterNew && isIDLE) {
            if (historyMessage.pFlags.unread && !historyMessage.pFlags.out &&
              lastIsRead) {
    ...
    $rootScope.$watch('idle.isIDLE', function (newVal) {
      if (!newVal &&
        $scope.curDialog &&
        $scope.curDialog.peerID && !$scope.historyFilter.mediaType && !$scope.historyState.skipped) {
        AppMessagesManager.readHistory($scope.curDialog.peerID)
      }
      if (!newVal) {
  ...
  .controller('AppImSendController', function ($rootScope, $q, $scope, $timeout, MtpApiManager, Storage, AppProfileManager, AppChatsManager, AppUsersManager, AppPeersManager, AppDocsManager, AppMessagesManager, AppInlineBotsManager, MtpApiFileManager, DraftsManager, RichTextProcessor) {
    ...
    $scope.$on('draft_updated', function (e, draftUpdate) {
      if (draftUpdate.peerID == $scope.curDialog.peerID && !draftUpdate.local) {
        getDraft()
      }
    })
    ...
    function replyClear(byUser) {
      ...
      if (mid &&
        ...
        $scope.historyState.replyKeyboard.mid == mid && !$scope.historyState.replyKeyboard.pFlags.hidden) {
